...
L'applicativo Python realizzato, sia in REST che GraphQL, simula la gestione delle prenotazioni dei servizi di un club sportivo. È composto precisamente da due microservizi che comunicano in maniera sincrona tra di loro:  
...
```bash
curl -X DELETE http://localhost:5001/resources/campo/RSSMRA85M01H501Z/2025-09-20/10/tennis
```

Eliminazione di una prenotazione in piscina  

```bash
curl -X DELETE http://localhost:5001/resources/piscina/RSSMRA85M01H501Z/2025-09-11 
```
...
```@strawberry.input``` è un tipo specifico quando vengono forniti dati in input ad una richiesta
...
In questo esempio si sta invocando la query con resolver ```checkMember``` che richiede l'argomento ```cf```, vengono specificati i campi di output desiderati, non è obbligatorio inserirli tutti (```cf```, ```name```, ```surname```, ```registration_date```), ma solo quelli a cui si è interessati.
...
In questo esempio si sta invocando la mutazione con resolver ```addMember``` che richiede il tipo input  ```member``` con tutti i suoi campi obbligatori.
